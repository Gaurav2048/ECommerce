...
public class ExclusiveAdapter extends RecyclerView.Adapter<ExclusiveAdapter.viewHolder> implements ProductActions {
   Context context;
   boolean isShimmering= false;
   List<Product> productList;
   ClickListner clickListner;

    public ExclusiveAdapter(@NonNull Context context, ClickListner clickListner) {
    ...
    public void makeDatabaseCall(){
        ...
        category categoory = ((MainActivity)context).getCaterogy();
        if(categoory != null)
        {
            new ProductController(this).get_new_popular_productsByCategory("2","0", categoory.getmCategoryid());
        }
    }
